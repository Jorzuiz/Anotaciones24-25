...
protected:
    ...
    struct TreeNode {
        ...
        Link iz, dr;
        int altura;
        int tam; // Metemos el numero de nodos que tiene en el lado izquierdo + 1
        TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
            int alt = 1, int t = 1) : elem(e), iz(i), dr(d), altura(alt), tam(t) {}
    };
...
public:
    ...
    bool insert(T const& e) {
        if (raiz != nullptr) return inserta(e, raiz, raiz->tam);
...
protected:
    T const& k_esimo(int k, Link const& a) const {
        if (k == a->tam)    // Si encontramos la primera k posicion
            return a->elem;
        if (k < a->tam)  // Si no, seguimos bajando
            return k_esimo(k, a->iz);
        
        return k_esimo(k - a->tam, a->dr); // Si no lo encontramos en la rama izquierda, nos movemos a la derecha 
    }
    ...
    bool inserta(T const& e, Link& a, int altura = 1) { //Link = TreeNode*
        bool crece;
        if (a == nullptr) { // se inserta el nuevo elemento e
            a = new TreeNode(e); //defecto a 1
            ++nelems;
            crece = true;
        }
