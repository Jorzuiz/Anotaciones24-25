...
def test_cost_iterative(x_train, y_train):
    ...
    print("----Compute iterative cost POO-------")
    compute_cost_test(cost_test_obj)
...
def test_gradient_descent(x_train, y_train, w, b):
    ...
    #probar con 0,01 823
    w,b,j_history,w_initial,b_initial=lr.gradient_descent(0.01,1500)
    predict1 = 3.5 * w + b
    print('\033[0mfor score 3.5, we predict user score of (3.97) %.2f' %
          predict1)
    print(predict1)
    print("Case 1")
    assert np.isclose(predict1, 3.96667965 ), f"Cost must be 3.966 for a perfect prediction but got {predict1}"
    print("Case 1 passed!")
    ...
    print("Case 2")
    print(predict2)
    assert np.isclose(predict2, 6.85867675 ), f"Cost must be 6.858 for a perfect prediction but got {predict2}"
