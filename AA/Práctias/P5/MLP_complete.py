...
class MLP:
    ...
    def __init__(self, inputLayer, hiddenLayers, outputLayer, seed=0, epislom = 0.12):
        ...
        self.numHiddenLayers = hiddenLayers.size[0] # Numero de capas ocultas
        self.theta = []
        
        self.layers = [inputLayer] + self.numHiddenLayers + [outputLayer]
        # Inicializar pesos para las capas ocultas
        for i in range(self.numHiddenLayers - 1):
            theta = np.random.uniform(-epislom, epislom, 
                                       (hiddenLayers[i+1], hiddenLayers[i] + 1))
    ...
    def feedforward(self,x):
        ...
        for theta in self.theta:
            z = np.dot(a1, self.theta1.T)                  # Activación de la primera capa
            a = self._sigmoid(z2)	                        # Normalizacion de la capa oculta
            a = np.hstack([np.ones((z2.shape[0], 1)), a2])	# Adicion de neurona de sesgo
